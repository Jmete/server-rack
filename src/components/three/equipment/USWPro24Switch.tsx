...
interface USWPro24SwitchProps {
  equipment: Equipment;
  onClick?: () => void;
  isSelected?: boolean;
}

// USW Pro 24 PoE dimensions (same chassis as 48-port)
...
export function USWPro24Switch({
  equipment,
  onClick,
  isSelected,
}: USWPro24SwitchProps) {
  ...
  // Port layout for USW Pro 24 PoE
  // 24 RJ45 ports: 12 on top row, 12 on bottom row
  // Ports are shifted significantly to the right (large gap after touchscreen)
  const portAreaStart = -FACEPLATE_WIDTH / 2 + mmToScene(210); // Shifted right - more space after display
  const rj45Spacing = mmToScene(15); // Same spacing as 48-port model
  const rowSpacing = mmToScene(10); // Vertical spacing between rows
  const rj45Span = rj45Spacing * 11; // 12 ports = 11 gaps
  const rj45PanelWidth = rj45Span + mmToScene(24);
  const rj45PanelCenterX = portAreaStart + rj45Span / 2;

  // SFP+ ports on far right - single column, 2 stacked vertically
  ...
  return (
    <group position={[0, yPosition, zPosition]} onClick={handleClick}>
      ...
      {/* RJ45 LAN Ports - 24 ports in 2 rows of 12 */}
      {ports
        .filter((p) => p.type === "rj45-lan")
        .slice(0, 24)
        .map((port, index) => {
          // Layout: ports 1-12 on top row, ports 13-24 on bottom row
      ...
      {/* SFP+ dark panel - narrower for 2 ports in single column */}
      <mesh position={[sfpAreaStart, 0, frontZ - mmToScene(0.5)]}>
        <boxGeometry args={[mmToScene(18), HEIGHT * 0.65, mmToScene(2)]} />
        <meshStandardMaterial color="#1a1a1a" />
      </mesh>

      {/* SFP+ Ports - 2 ports stacked vertically */}
      {ports
        .filter((p) => p.type === "sfp-plus")
        .map((port, index) => {
          // Single column, 2 rows: top then bottom
          return (
            <SFPPort
              key={port.globalId}
              port={port}
